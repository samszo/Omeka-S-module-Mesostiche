...
, arrSourceTexte = [
    ...
    ,{'label':'Cantiquest - Chants Joyeux', 'gen':false,'fct':txtFromCsv,'params':{'url':'https://samszo.github.io/mesostiche/data/chants-joyeux.csv'}}
    ,{'label':'Cantiquest - Chants de Victoire', 'gen':false,'fct':txtFromJson,'params':{'url':'https://samszo.github.io/mesostiche/data/chants-victoire.json'}}
    ,{'label':'Poésie française', 'gen':false,'fct':txtFromOmk,'params':{'data':aleaItems}}
]
...
function txtFromCsv(e, d){
    ...
    d3.csv('https://samszo.github.io/mesostiche/data/chants-joyeux.csv').then(function(data) {
        //création des textes 
        data.forEach(s=>{
            let st = s.txt;
            if(st!=""){
...
function txtFromJson(e, d){
    ...
    d3.json('https://samszo.github.io/mesostiche/data/chants-victoire.json').then(function(data) {
        //création des textes 
...
function txtFromOmk(e, d){

    textes = []
    aleaItems.forEach(i => {
        textes.push(i['o:title']);
    });
    nettoie=true;
    rdmTexte=true;
    regleVide=false;            
